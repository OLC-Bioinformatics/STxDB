Notes:
•	Novel Alleles: FASTA headers were renamed to indicate novel allele identification by StxTyper. Subtypes are marked as "Novel" or, if closely related to a known type, appended with _like (e.g., stx2f_like).
•	Partial Alleles: FASTA headers were renamed to indicate partial allele identification by StxTyper. These alleles show >99% identity to known subtypes but differ at the C-terminus. They are appended with _part according to their closest match (e.g., stx1a_part).
•	Removed Entry: Removed StxOp4769_Stx2_1353 as raw-read analysis indicates it is an assembly artifact.


Fasta_Header	Operon sequence ID	ToxinType	Stx profile
